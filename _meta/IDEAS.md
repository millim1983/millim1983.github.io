...
| A3 | `coding-education-01-trash-sorter` | coding-education | **아이와 만드는 분리수거 도우미** (설명서형 튜토리얼). Teachable Machine으로 캔/페트/종이 사진을 아이가 직접 모아 노코드 이미지 분류 모델 학습 → Claude로 바이브코딩한 웹페이지에 연결해 웹캠 인식 → (선택) 라즈베리파이+서보/LED로 피지컬 AI 확장. "의도 정의+결과 검증"을 아이가 체험하는 게 목표(PERSONA §1·§9). 단계별 캡처·코드. | **글감 확정 (2026-06-20 사용자 지시)** — 신규 큐 | 높 |
...
리뷰글이 다뤄야 할 6가지를 모두 채운다. 이게 리뷰의 정체성.
