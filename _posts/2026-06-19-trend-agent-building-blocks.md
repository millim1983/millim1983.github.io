...
이 규격은 빠르게 사실상 표준이 됐다. 공식 MCP 레지스트리에 2026년 5월 기준 약 9,650개의 서버가 등록돼 있다(2025년 말 약 6,800개에서 반년 만에 +38%). SDK는 2026년 3월 기준 월 9,700만 건 내려받히고, Stacklok의 2026 조사에서 소프트웨어 조직의 41%가 MCP 서버를 제한적·전면 프로덕션에 쓴다고 답했다. Anthropic·OpenAI·Google·Microsoft·Salesforce(Agentforce)·Cloudflare·Replit이 모두 지원을 붙였다. 공개 서버만 500개가 넘어 Postgres·MySQL·SQLite 같은 DB, Google Drive·Box, Slack, Jira·Asana 같은 일상 도구를 감싼다.
...
스킬의 핵심 성질은 필요할 때만 불러온다는 점이다. 모든 규칙을 매 대화에 욱여넣으면 맥락이 금세 포화한다. 스킬은 평소에는 이름과 한 줄 설명(description)만 떠 있다가, 관련 작업이 들어올 때 본문이 펼쳐진다. Anthropic의 Agent Skills가 이 방식으로, 작업이 들어오면 Claude가 등록된 스킬들의 description을 훑어 맞는 것만 전체 지침을 로드한다. 덕분에 수십 개의 전문 절차를 등록해 두고도 맥락을 가볍게 유지한다. 실제 효과도 측정된다. 데이터 분석 도구 dbt의 Agent Skills는 데이터 모델링 작업의 정확도를 벤치마크에서 끌어올린 사례로 보고됐다.
...
훅(hook)은 에이전트 동작의 특정 시점에 자동으로 실행되는 스크립트다. Claude Code의 경우 17개 이벤트(도구 호출 직전·직후, 세션 시작, 응답 완료 등)에 4가지 타입의 훅을 걸 수 있다. 도구를 호출하기 직전, 작업을 마친 직후 같은 이벤트에 코드를 건다. 프롬프트와의 결정적 차이가 여기 있다. 프롬프트는 모델의 해석에 의존하지만, 훅은 정해진 코드를 그대로 실행한다.
...
이 지휘를 떠받치는 프레임워크는 2026년 들어 대략 여섯 개로 수렴했다.

| 프레임워크 | 오케스트레이션 방식 | 특징·수치 |
|---|---|---|
| LangGraph (LangChain) | 그래프 기반(노드·엣지) | 감사 로그·롤백·체크포인트, LangSmith 관측. 2026 초 GitHub 스타에서 CrewAI 추월, 프로덕션 티어 |
| CrewAI | 역할(role) 기반 멀티에이전트 | 스타 44,600+·포춘500의 약 60% 채택. 단순 작업에선 LangGraph 대비 토큰 최대 3배 |
| Microsoft Agent Framework | AutoGen+Semantic Kernel 통합 | 2026년 4월 v1.0 정식 출시 |
| OpenAI Agents SDK | 코드 중심 + MCP 기본 지원 | 2026년 4월 개편: 샌드박싱·서브에이전트·파일시스템 도구 추가 |
| Google ADK | 멀티언어 실행 프레임워크 | Python·Java·Go·TypeScript, I/O '26서 ADK 2.0 |
| Claude Agent SDK / AWS Strands | 각 사의 에이전트 SDK | Anthropic / AWS |

공통점은 분기·재시도·상태를 코드로 고정한다는 것이고, 차이는 그 흐름을 그래프로(LangGraph) 짤지 역할로(CrewAI) 짤지다. 프레임워크 선택이 곧 오케스트레이션을 어디까지 코드로 굳힐지의 선택이 된다.
